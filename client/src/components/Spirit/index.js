...
function Dolly() {
  useFrame(state => {
    state.camera.position.z = 20 + Math.sin(state.clock.getElapsedTime()) * 20
    ...
    <Canvas pixelRatio={window.devicePixelRatio} camera={{ position: [0, 0, 60] }}>
      ...
      <Detailed distances={[0, 50, 100]}>
        <mesh>
          ...
          <meshBasicMaterial attach="material" color="lightgrey" wireframe />
        </mesh>
        <mesh>
          <icosahedronBufferGeometry attach="geometry" args={[10, 1]} />
